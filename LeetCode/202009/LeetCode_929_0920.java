...
public class LeetCode_929 {
    public int numUniqueEmails(String[] emails) {
        ...
        for (String email : emails) {
            set.add(ChEmail(email));
        }
        return set.size();
    }

    public String ChEmail(String str){
        int at = str.indexOf('@');
        int ad = str.indexOf('+');
        StringBuilder sb = new StringBuilder();
        if (ad != -1){
            sb.append(str.substring(0,ad).replace(".",""));
        }else {
            sb.append(str.substring(0,at).replace(".",""));
        }
        sb.append(str.substring(at));
        return sb.toString();
    }
    @Test
    public void test(){
        String[] s1 = {"test.email+alex@leetcode.com",
                "test.e.mail+bob.cathy@leetcode.com",
                "testemail+david@lee.tcode.com"};
        System.out.println(numUniqueEmails(s1));
    }
